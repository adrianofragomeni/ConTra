...
def train_epoch(training_loader,device, optimizer,net, loss, scheduler):
    ...
    for i_batch, sample_batched in enumerate(training_loader):
        ...
        if args_.length_context_video == 1 and args_.length_context_text == 1:

            text_embed,video_embed = net(text_features,video_features,branch="all")
        
        elif args_.length_context_video > 1 and args_.length_context_text == 1:
            ...
            negative_embed_video=net(None,negatives_features_video,branch="video")
             ...
             negatives_features_text=list(filter(None,itertools.chain(*np.array(sample_batched["negatives_texts"],dtype=object).T.tolist())))
             mask=sample_batched["masking"]
...
def validation_epoch(testing_loader,device, eval_,net, loss):
...
def train():
    ...
    training_set=FeatureLoader(path_dataframe=Path(args_.path_dataframes) / args_.dataset / "retrieval_train.csv",
                               path_features=Path(args_.path_features) / args_.dataset / "training_video_features.pkl",
