...
class FeatureLoader(Dataset):
    ...
    def __init__(self,path_dataframe,path_features,dataset,len_video,len_text,transform=None):
        ...
        self.df=pd.read_csv(path_dataframe).values
        self.dataset=dataset
        
        with open(path_features, 'rb') as fp:
            self.video_feat = np.array(pickle5.load(fp)["features"])
    ...
    def __getitem__(self,sequence_idx):
        ...
        if self.len_text == self.len_video:
            sample= {"video_embed": self.video_feat[idx_sequence,:], "text_embed": self.narrations[list(idx_sequence)].tolist(),"negatives_texts":negative_texts,"negatives_videos":negative_videos,"masking":mask}
        elif self.len_text==1:
            sample= {"video_embed": self.video_feat[idx_sequence,:], "text_embed": self.narrations[[idx]].tolist(), "negatives_videos":negative_videos,"masking":mask}
        elif self.len_video==1:
            sample= {"video_embed": self.video_feat[idx,:], "text_embed": self.narrations[list(idx_sequence)].tolist(), "negatives_texts":negative_texts,"masking":mask}    
