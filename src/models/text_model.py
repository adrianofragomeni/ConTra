...
class Sentence_Embedding(nn.Module):
    def __init__(
        ...
        token_to_word = np.load(Path(args_.path_resources) /"s3d_dict.npy")
        for i, t in enumerate(token_to_word):
            self.word_to_token[t] = i + 1
            
        self.missingword_to_token = {}
        token_to_missingword = np.load(Path(args_.path_resources) / args_.dataset / "missing_words.npy")
        self.embedding_missingword=np.load(Path(args_.path_resources) / args_.dataset / "embeddings_missing_words.npy")
